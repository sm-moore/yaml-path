...
function parseDocumentYaml({ document, selection }: vscode.TextEditor): string {
	const selectedLine = document.lineAt(selection.active);
	const lastSelectedLineNumber = selection.end.line;

	if (
		selectedLine.isEmptyOrWhitespace ||
		util.isCommentLine(selectedLine.text)
	) {
		return "";
	}
...
function parseDocumentYamlAtPosition(
	document: vscode.TextDocument,
	position: vscode.Position
): string {
	const selectedLine = document.lineAt(position);
	const range = new vscode.Range(0, 0, position.line, selectedLine.text.length);
	const lines: string[] = document.getText(range).split("\n");
	return util.parseYaml(selectedLine.text, lines);
}

export { parseDocumentYaml, parseDocumentYamlAtPosition };
